...
tr.head()
plotPerColumnDistribution(tr, 100, 5)
plotScatterMatrix(tr, 25, 15)
...
def DecisionTree():
    if len(NameEn.get()) == 0:
        ...
        c.execute(
            ...
            (NameEn.get(), AgeEn.get(), Symptom1.get(), Symptom2.get(), Symptom3.get(), Symptom4.get(), Symptom5.get(), pred2.get()))
        conn.commit()
        c.close()
        conn.close()
...
root.title('(DPS)Disease Prediction System')
root.resizable(0, 0)
...
def Exit():
    qExit = messagebox.askyesno("System", "Are you sure you want to exit the system")
    if qExit:
        root.destroy()
        exit()
...
w2 = Label(root, justify=LEFT, text="Disease Prediction System ", fg="Red", bg="Ivory")
w2.config(font=("Times", 30, "bold italic"))
w2.grid(row=1, column=0, columnspan=2, padx=100)
w2 = Label(root, justify=LEFT, text="DPS: Shadman,Jobiar,Enia", fg="Pink", bg="Ivory")
w2.config(font=("Times", 30, "bold italic"))
...
AgeLb = Label(root, text="Age of the Patient", fg="Red", bg="Ivory")
...
AgeEn = Entry(root, textvariable=Name)
...
S3.grid(row=10, column=1)

S4 = OptionMenu(root, Symptom4, *OPTIONS)
S4.grid(row=11, column=1)

S5 = OptionMenu(root, Symptom5, *OPTIONS)
S5.grid(row=12, column=1)
